...
namespace Myfunctions
{
	...
	int MiniumNumberInMatrix(int matrix1[3][3], short row, short colo)
	{
		int mininumber = matrix1[0][0];
		for (int i = 0; i < row; i++)
		{
			for (int j = 0; j < colo; j++)
			{
				if (matrix1[i][j] < mininumber)
				{
					mininumber = matrix1[i][j];
				}
			}
		}
		return mininumber;
	}
	int MaxiumNumberInMatrix(int matrix1[3][3], short row, short colo)
	{
	 ...
	 bool IspalindromeMatrix(int matrix[3][3], short row, short colo)
	{
		
		for (int i = 0; i < row; i++)
		{
			for (int j = 0; j < colo/2; j++)
			{
				if (matrix[i][j] != matrix[i][colo - 1 - j])
				{
					return false;
				}
